...
def getExternalIP(ip_ver=''):
    ...
    def getip(url):
        try:
            ip = requests.get(url, timeout=5.0).text.strip()
            if isValidIP(ip):
                return ip
        except:
            pass
...
def ipToHost(ip, exclude_dsip_aliases=True):
    ...
    :type ip:                       str
    :param exclude_dsip_aliases:    Whether to exclude dsip specific DNS aliases
    :type exclude_dsip_aliases:     bool
    ...
    try:
        ...
        hostnames = [addr_info[0], *addr_info[1]]
        if exclude_dsip_aliases:
            hostnames = [x for x in hostnames if x not in DSIP_DNS_ALIASES]
        return hostnames[0]
    except:
        return None
